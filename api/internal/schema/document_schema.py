...
class CreateDocumentsReq(FlaskForm):
    ...
    def validate_upload_file_ids(self, field: ListField) -> None:
        """校验上传文件id列表，只要以validate开头加上字段名字，函数就会被自动调用"""
        # 1.校验数据类型与非空
        if not isinstance(field.data, list):
            raise ValidationError("文件id列表格式必须是数组")

        # 2.校验数据的长度，最长不能超过50条记录
        if len(field.data) == 0 or len(field.data) > 50:
            raise ValidationError("新增的文档数范围在0-50")
    ...
    def validate_rule(self, field: DictField) -> None:
        ...
        if self.process_type.data == ProcessType.AUTOMATIC:
            ...
            if "separators" not in field.data["segment"] or not isinstance(field.data["segment"]["separators"], list):
                raise ValidationError("分隔列表不能为空且为列表")
            ...
            if field.data["segment"]["chunk_size"] < 200 or field.data["segment"]["chunk_size"] > 8000:
                raise ValidationError("分割块大小在200-8000")
            ...
            max_overlap = min(int(field.data["segment"]["chunk_size"] * 0.5), 1000)
            if not (0 <= field.data["segment"]["chunk_overlap"] <= max_overlap):
                raise ValidationError(f"块重叠大小在0-{max_overlap}")
...
class GetDocumentWithPageResp(Schema):
    """获取文档分页列表响应结构"""
    id = fields.UUID(dump_default="")
    name = fields.String(dump_default="")
    character_count = fields.Integer(dump_default=0)
