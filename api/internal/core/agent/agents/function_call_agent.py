...
class FunctionCallAgent(BaseAgent):
    ...
    def _llm_node(self, state: AgentState) -> AgentState:
        ...
        try:
            for chunk in llm.stream(state["messages"]):
                ...
                # 5.检测生成类型是工具参数还是文本生成（工具调用优先级高于文本）
                # TODO: 这里是为了兼容DeepseekV4在非思考模式下使用工具调用，在流式输出时，第一个chunk不返回tool_calls而返回自然语言的问题。
                # bug地址：https://github.com/deepseek-ai/DeepSeek-V3/issues/1244?utm_source=chatgpt.com
                if chunk.tool_calls:
            ...
            logger.exception("LLM节点发生错误, 错误信息", error=str(e))
            self.agent_queue_manager.publish_error(state["task_id"], f"LLM节点发生错误, 错误信息: {str(e)}")
            raise e

        # 7.1 守卫：LLM流式输出为空时提前终止，避免后续None引用崩溃
        if gathered is None:
            logger.error("llm_stream_empty", task_id=str(state["task_id"]))
        ...
        if generation_type == "thought":
            logger.info(
                "llm_decided_tool_calls",
                tool_calls=[{"name": tc["name"], "args": tc["args"]} for tc in gathered.tool_calls],
    ...
    def _tools_node(self, state: AgentState) -> AgentState:
        ...
        logger.info("tools_node_started", tool_call_count=len(tool_calls), tools=[tc["name"] for tc in tool_calls])
        ...
        for tool_call in tool_calls:
            ...
            id = uuid.uuid4()
            start_at = time.perf_counter()
            logger.info("tool_invoking", tool_name=tool_call["name"], tool_input=tool_call["args"])
            ...
            try:
                ...
                logger.info(
                    ...
                    latency=round(time.perf_counter() - start_at, 3),
                    result_len=len(json.dumps(tool_result, ensure_ascii=False)),
                )
            except Exception as e:
                # 6.添加错误工具信息
                logger.exception("工具执行失败", tool_name=tool_call["name"])
