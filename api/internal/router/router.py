...
class Router:
    ...
    def register_router(self, app: Flask):
        ...
        bp.add_url_rule(
            "/mcp-tools",
            view_func=self.mcp_tool_handler.get_mcp_tool_providers_with_page,
        )
        bp.add_url_rule(
            "/mcp-tools/validate-mcp-schema",
            methods=["POST"],
            view_func=self.mcp_tool_handler.validate_mcp_schema,
        )
        bp.add_url_rule(
            "/mcp-tools",
            methods=["POST"],
            view_func=self.mcp_tool_handler.create_mcp_tool_provider,
        )
        bp.add_url_rule(
            "/mcp-tools/<uuid:provider_id>",
            view_func=self.mcp_tool_handler.get_mcp_tool_provider,
        )
        bp.add_url_rule(
            "/mcp-tools/<uuid:provider_id>",
            methods=["POST"],
            view_func=self.mcp_tool_handler.update_mcp_tool_provider,
        )
        bp.add_url_rule(
            "/mcp-tools/<uuid:provider_id>/tools/<string:tool_name>",
            view_func=self.mcp_tool_handler.get_mcp_tool,
        )
        bp.add_url_rule(
            "/mcp-tools/<uuid:provider_id>/delete",
            methods=["POST"],
            view_func=self.mcp_tool_handler.delete_mcp_tool_provider,
        )
