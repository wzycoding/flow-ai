...
class AgentQueueManager:
    ...
    def listen(self, task_id: UUID) -> Generator:
        ...
        while True:
            try:
                # 3.从队列中提取数据并检测是否存在，如果存在则使用yield关键字返回，这里超时会抛出queue.Empty异常而不是返回None
    ...
    def publish(self, task_id: UUID, agent_thought: AgentThought) -> None:
        ...
        # 2.检测事件类型是否是需要停止的类型，涵盖STOP、ERROR、TIMEOUT、AGENT_END
    ...
    @classmethod
    def set_stop_flag(cls, task_id: UUID, invoke_from: InvokeFrom, user_id: UUID, redis_client: Redis) -> None:
