...
from internal.core.workflow.entities.node_entity import NodeStatus, NodeType, BaseNodeData
...
class WorkflowService(BaseService):
    ...
    def update_draft_graph(self, workflow_id: UUID, draft_graph: dict[str, Any], account: Account) -> Workflow:
        ...
        # 3.更新工作流草稿图配置；仅布局位置变化不影响运行逻辑，因此保留调试通过状态
        is_debug_sensitive_changed = self._is_debug_sensitive_graph_changed(
            workflow.draft_graph,
            validate_draft_graph,
        )
        self.update(workflow, **{
            "draft_graph": validate_draft_graph,
            "is_debug_passed": bool(workflow.is_debug_passed) and not is_debug_sensitive_changed,
        })
        ...
        for node in validate_draft_graph["nodes"]:
            if node.get("node_type") == NodeType.TOOL:
                ...
                if node.get("tool_type") == "builtin_tool":
                    ...
                    tool_record = self.db.session.query(McpTool).filter(
                        McpTool.provider_id == node.get("provider_id"),
    ...
    def debug_workflow(self, workflow_id: UUID, inputs: dict[str, Any], account: Account) -> Generator:
        ...
        def handle_stream() -> Generator:
            ...
            def update_workflow_debug_status(is_debug_passed: bool) -> None:
                workflow_record = self.db.session.get(Workflow, workflow.id)
                if workflow_record:
                    self.update(workflow_record, **{
                        "is_debug_passed": is_debug_passed,
                    })
            ...
            try:
                ...
                update_workflow_debug_status(True)
                ...
                update_workflow_debug_status(False)
                raise
    ...
    @classmethod
    def _normalize_graph_for_debug(cls, graph: dict[str, Any]) -> dict[str, Any]:
    ...
    def _validate_graph(self, workflow_id: UUID, graph: dict[str, Any], account: Account) -> dict[str, Any]:
        ...
        for node in nodes:
            try:
                ...
                logger.warning("workflow_node_skipped", node_id=str(node.get("id")), error=str(e))
                continue
        ...
        for edge in edges:
            try:
                ...
                logger.warning("workflow_edge_skipped", edge_id=str(edge.get("id")), error=str(e))
                continue
