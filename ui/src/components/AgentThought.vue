  ...
  <div :class="`flex flex-col rounded-2xl border ${visible ? 'w-[320px]' : 'w-[180px]'}`">
    ...
    <a-collapse class="agent-thought" v-if="visible" destroy-on-hide :bordered="false">
      ...
      >
        ...
        <template #header>
          <div style="color: #374151 !important;" v-if="agent_thought.event === QueueEvent.longTermMemoryRecall">
            长期记忆召回
          </div>
          <div style="color: #374151 !important;" v-if="agent_thought.event === QueueEvent.agentThought">
            智能体推理
          </div>
          <div style="color: #374151 !important;" v-if="agent_thought.event === QueueEvent.datasetRetrieval">
            搜索知识库
          </div>
          <div style="color: #374151 !important;" v-if="agent_thought.event === QueueEvent.agentAction">
            调用工具
          </div>
          <div style="color: #374151 !important;" v-if="agent_thought.event === QueueEvent.agentMessage">
            智能体消息
          </div>
        </template>
        <template #extra>
