        ...
        with flask_app.app_context():
            try:
                ...
                agent = self.agent_runner.create_agent(
                    llm, app_config, tools, app.account_id, InvokeFrom.SERVICE_API,
                )
                ...
                # 11.异常时更新消息状态为ERROR，避免用户永远看到"处理中"
                logger.exception(
                    "wechat_thread_chat_failed",
                    app_id=str(app_id),
                ...
                if message:
                    self.update(message, status=MessageStatus.ERROR, error=str(e))
